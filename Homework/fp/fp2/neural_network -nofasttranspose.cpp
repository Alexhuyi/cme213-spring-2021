...
void parallel_feedforward(NeuralNetwork &nn, real *d_X, NNcache &nncache, int size_per_proc){
    ...
    myGEMM(nncache.W1, d_X, nncache.a1, &alpha,&beta, H, size_per_proc, M ,false,false);
    ...
    myGEMM(nncache.W2, nncache.a1, nncache.y_pred, &alpha,&beta, C, size_per_proc, H,false,false);
...
void parallel_backprop(NeuralNetwork& nn, real *d_X, real *d_Y, real reg, NNcache &nncache,int batch_size, int size_per_proc, int num_procs){
    ...
    gpu_addmat_inplace(nncache.dW2,nncache.W2,0,1.0,C,H);
    myGEMM(nncache.diff,nncache.a1,nncache.dW2,&alpha,&reg,C,H,size_per_proc,false,true);
    ...
    real beta = 0.0;
    myGEMM(nncache.W2,nncache.diff,nncache.da1,&alpha,&beta,H,size_per_proc,C,true,false);
    ...
    gpu_addmat_inplace(nncache.dW1,nncache.W1,0,1.0,H,M);
    myGEMM(nncache.dz1, d_X, nncache.dW1, &alpha, &reg, H, M, size_per_proc,false,true);
...
void parallel_gradientdecent(NeuralNetwork& nn, NNcache &nncache, real learning_rate){
    ...
    //update nncache params 
    gpu_addmat_inplace(nncache.W1,nncache.dW1,1.0,-learning_rate,H,M);
    gpu_addmat_inplace(nncache.W2,nncache.dW2,1.0,-learning_rate,C,H);
    gpu_addmat_inplace(nncache.b1,nncache.db1,1.0,-learning_rate,H,1);
    gpu_addmat_inplace(nncache.b2,nncache.db2,1.0,-learning_rate,C,1);
}
  ...
  */
  ...
  for (int epoch = 0; epoch < epochs; ++epoch) {
    for (int batch = 0; batch < num_batches; ++batch) {
      ...
      // // MPI_Allreduce, MPI_IN_PLACE IS FASTER 
      MPI_SAFE_CALL(MPI_Allreduce(MPI_IN_PLACE, h_dW1, H * M, MPI_FP, MPI_SUM, MPI_COMM_WORLD));
      MPI_SAFE_CALL(MPI_Allreduce(MPI_IN_PLACE, h_dW2, C * H, MPI_FP, MPI_SUM, MPI_COMM_WORLD));
      MPI_SAFE_CALL(MPI_Allreduce(MPI_IN_PLACE, h_db1, H, MPI_FP, MPI_SUM, MPI_COMM_WORLD));
      MPI_SAFE_CALL(MPI_Allreduce(MPI_IN_PLACE, h_db2, C, MPI_FP, MPI_SUM, MPI_COMM_WORLD));

      // // cudaMemcpy's, cudaMemcpyHostToDevice
      cudaMemcpy(nncache.dW1, h_dW1, H * M * sizeof(real), cudaMemcpyHostToDevice);
      cudaMemcpy(nncache.dW2, h_dW2, C * H * sizeof(real), cudaMemcpyHostToDevice);
      cudaMemcpy(nncache.db1, h_db1, H * sizeof(real), cudaMemcpyHostToDevice);
      cudaMemcpy(nncache.db2, h_db2, C * sizeof(real), cudaMemcpyHostToDevice);
