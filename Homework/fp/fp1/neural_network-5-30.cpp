...
void parallel_backprop(NeuralNetwork& nn, real *d_X, real *d_Y, real reg, NNcache &nncache,int batch_size, int size_per_proc, int num_procs){
    ...
    real alpha = 1.0;
    cudaMemcpy(nncache.dW2, nncache.W2, sizeof(real) * C * H, cudaMemcpyDeviceToDevice);//magnitude have probelm?
...
void parallel_train(NeuralNetwork& nn, const arma::Mat<real>& X,
  ...
  */
  ...
  for (int batch = 0; batch < num_batches; ++batch) {
      ...
      arma::mat X_batch(M,scounts_X[rank]/M);
      ...
      arma::mat Y_batch(C ,scounts_Y[rank]/C);
      ...
      // cudaMalloc((void **)&d_X_batches[batch], M * nsample_this_proc * sizeof(real));
      // cudaMalloc((void **)&d_Y_batches[batch], C * nsample_this_proc * sizeof(real));
      // cudaMemcpy(d_X_batches[batch], X_batch.memptr(), M *  nsample_this_proc * sizeof(real), cudaMemcpyHostToDevice);
      // cudaMemcpy(d_Y_batches[batch], Y_batch.memptr(), C *  nsample_this_proc * sizeof(real), cudaMemcpyHostToDevice);   
